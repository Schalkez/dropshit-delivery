...
const breadcrumb: BreadcrumbProps = {
  items: [
    ...
    {
      key: webRoutes.users,
      title: <Link to={webRoutes.history_invest}>Histories Invest</Link>,
    },
  ],
};

const HistoriesInvest = () => {
  ...
  const columns: ProColumns[] = [
        ...
        <div className="flex flex-col justify-center items-center gap-1">
          ...
          <div className="flex items-center justify-between gap-1">
            {row?.transaction_status === 'pending' ? (
              <Tag color="warning">Earning</Tag>
            ) : row?.transaction_status === 'finish' ? (
              <Tag color={'green-inverse'}>Finish</Tag>
            ) : (
    ...
    },
    {
      ...
      ellipsis: true,
      render: (_, row: any) => (
        <div>{new Date(row?.createdAt).toLocaleDateString()} - {new Date(row?.time_end + new Date(row?.createdAt)?.getTime()).toLocaleDateString()}</div>
      ),
    },
...
export default HistoriesInvest;
